...
export const createStarWrap = (app: Application) => {
  const starTexture = app.loader.resources.star.texture;
  if (!starTexture) {
    throw new Error('Start texture not found');
  }
  ...
  const randomizeStar = (star: Star, initial = false) => {
  ...
  return () => (warpSpeed = warpSpeed > 0 ? 0 : 1);
};
